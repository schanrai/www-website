/* ============================================================
   ARCHIVED DESIGN HANDOFF — NOT PART OF THE LIVE SITE

   Purpose & origin
   This file was the original design handoff for the site: the
   approved color tokens, image paths, and final copy (in the
   CONTENT object below), plus throwaway React components used
   only to render "Variation D Preview.html". Per README.md in
   this folder, the CONTENT object was the input to be extracted
   into typed constants rather than hardcoded into components.

   Status: superseded. Do NOT edit this file to change the site.
   Nothing in src/ imports it, and Astro builds only from src/
   and public/, so it is never compiled or served. The copy here
   is stale and no longer matches the live site.

   Live sources of truth:
     - Copy   → src/content/{homepage,menopause,seniors}.ts
     - Tokens → src/styles/global.css

   Why retained
   It is the historical record of the original design intent —
   the approved palette, type scale, and first-round copy — and
   its sibling README.md is still cited by src/styles/global.css
   as the source of truth for the design system. Keep the folder
   for that reference; treat this file as read-only history.
   ============================================================ */
